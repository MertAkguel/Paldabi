...
protected:
    ...
    */
    virtual void alignCheck_(
            const size_t text_pos) const {}; // leave this function empty. Just call it whenever you check an alignment.


    // your data members (and helper functions, if you need them), go here:
    //  ...
    //
    std::unordered_map<char, uint32_t> dict;
    std::string pattern_;
    std::vector<size_t> alignment_positions;

    bool check_in_pattern(const char last_char) const {
        for (auto &letter: pattern_) {
            if (last_char == letter) {
                return true;
            }
        }
