...
struct ACNode {
    ...
    ACNode() {
        suffix_link = parent_link = output_link = depth = 0;
        terminal = false;
    }

    unsigned suffix_link;
    unsigned output_link;
    unsigned parent_link;
    std::map<char, unsigned> children{};
    std::vector<unsigned> needle_indices{};
    unsigned depth;
    bool terminal;
    char letter{};
};
...
private:
    ...
    bool delta(const char letter, std::map<char, unsigned> &children, unsigned &i) {

        // std::map<char, int>::const_iterator
        auto found = children.find(letter);
