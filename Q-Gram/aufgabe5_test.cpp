...
TRT test_getHits()
{
  ...
  auto r2 = qgi.getHits(63); // = TTT
  if (r.empty() && r2.empty()) ++points;

  r = qgi.getHits(35); // = GAT  2*4^2 + 0 + 3
  if (r.size() == 2 && r[0] == 07 && r[1] == 0) points += 2;

  r = qgi.getHits(4); // = ACA  0 + 1*4^1 + 0
  if (r.size() == 2 && r[0] == 11 && r[1] == 4) ++points;
...
TRT test_exceptions()
{
  ...
  return make_pair((int)std::round(cases * 2 / 3), 2); // 1 case: 1pt; 2 cases: 1 pt; 3 cases 3 pts;
}

TRT test_BONUS(string filename)
{
...
int main(int argc, const char * argv[])
{
  ...
  report(points, &test_hash, "test_hash");
  report(points, &test_hashNext, "test_hashNext");
  report(points, &test_q_and_text, "test_q_and_text");
